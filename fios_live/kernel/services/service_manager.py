...
class ServiceManager:
    ...
    def run_autonomous_cycle(self) -> RepositoryState | None:
        ...
        The existing RepositoryMonitor detects a repository event.
        The Repository Brain is executed only when the monitor
        detects a repository change.

        When a change is detected, existing Builder Automation
        executes once and the monitor then consumes any generated
        repository changes.
        """
        ...
        brain_state = self.run_brain_cycle()

        automation_manager = (
            self.builder.automation_manager
        )
        ...
        automation_manager.execute()

        # Consume repository changes generated by automation so
        # they do not immediately retrigger the same cycle.
